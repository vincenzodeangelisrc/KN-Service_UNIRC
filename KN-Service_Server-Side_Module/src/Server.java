		...
		try {
			...
			if(type.equals("token")) {
				...
				if(obj.has("dataSN")) {
					...
					//Data for SN obtained through Primitive P1
					...
					if(typeData.equals("position_notification_request")) {
						...
						if(positionNotification2!=null) {
							...
							if(!commonC.isEmpty()) {
								...
								System.out.println("Error in linkedEph:"+hashEph+" "+hashEph2);
								}
				...
				if(count%100000==0) {
					...
					if(!found) {
						...
						System.out.println("There is a not-open session");
					}
			...
			try {
				...
				PrintWriter pw = new PrintWriter(new File("FILE_TO_PRINT_THE_EXCEPTION"));
